...
const recipePage = ({ params }: { params: { id: string } }) => {
  ...
  const { savedRecipes, addRecipe, removeRecipe } =
  ...
  const fetchRecipe = async () => {
    ...
    if (data && data.meals.length > 0) {
      ...
      const ingredientKeys = Object.keys(meal).filter((item) =>
    ...
    <div className="flex flex-col text-black p-6 mx-auto space-y-3 items-center">
      {recipe && (
        <div className="space-y-3">
          ...
          <h4 className="text-orange-700">Style: {recipe.strArea}</h4>
          <Image
          ...
          <Button
            buttonText={isRecipeSaved ? "Unsave recipe" : "Save recipe"}
